  ...
  context 'subscribed' do
    ...
    context '#check_subscribed_teams!' do
      ...
      it 'notifies past due subscription' do
        ...
        expect_any_instance_of(Team).to receive(:inform!).with(text: "Your subscription to StripeMock Default Plan ID ($4.99) is past due. #{team.update_cc_text}")
        subject.send(:check_subscribed_teams!)
      end
      it 'notifies past due subscription' do
        customer.subscriptions.data.first['status'] = 'canceled'
        expect(Stripe::Customer).to receive(:retrieve).and_return(customer)
        expect_any_instance_of(Team).to receive(:inform!).with(text: 'Your subscription to StripeMock Default Plan ID ($4.99) was canceled and your team has been downgraded. Thank you for being a customer!')
        subject.send(:check_subscribed_teams!)
