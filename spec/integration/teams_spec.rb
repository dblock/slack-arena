...
describe 'Teams', :js, type: :feature do
  ...
  context 'homepage' do
    ...
    it 'displays index.html page' do
      expect(title).to eq('Are.na integration with Slack')
    end

    it 'includes a link to add to slack with the client id' do
      expect(find("a[href='https://slack.com/oauth/authorize?scope=bot,commands&client_id=#{ENV.fetch('SLACK_CLIENT_ID', nil)}']"))
    end
  end
end
